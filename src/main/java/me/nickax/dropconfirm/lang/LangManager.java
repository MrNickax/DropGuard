...
public class LangManager {
    ...
    public void load() {
        ...
        if (!plugin.langFileCheck.checkExist("lang_" + lang)) {
            ...
            if (!plugin.langFileCheck.checkValid("lang_" + lang)) {
                if (!plugin.langFileCheck.checkValid("lang_en")) {
    ...
    private int loadedLanguages() {
        ...
        for (int id = 0; id < Arrays.stream(Objects.requireNonNull(folder.list())).count(); id++) {
            ...
            if (plugin.langFileCheck.checkValid(lang)) {
                v++;
            }
        }
        return v;
